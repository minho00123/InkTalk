...
# 🔥 도전
다수의 사용자가 서버에 접속해 한 캔버스에서 그림을 그리게 되면서 
동시 수정 에러를 뜻하는 ConcurrentModificationException이 발생했습니다. 

이는 사용자가 그린 한 획을 의미하는 Stoke들을 for문을 돌면서 읽어주는 과정에서 
...
### 첫번째 방법
여러 스레드가 동시에 공유된 리스트를 수정하면서 발생하는 문제였기 때문에 
synchronized()와 리스트에 Collections.synchronizedList() 메서드를 사용해 공유 자원을 동기화 하고자 하였습니다. 

하지만 다중 사용자의 복잡한 사용을 시행하는 경우에는 list에 접근하거나 순회하는 모든 코드를 동기화 처리해야했기 때문에 
코드가 간결하지 않고 유지보수가 쉽지 않았습니다.

### 두번째 방법
그렇기 때문에 List 의 구현체로 ArrayList가 아닌 CopyOnWriteArrayList를 사용해 
수정 작업이 발생할 때마다 원본 데이터를 새 배열로 복사해 작업하도록 하여 
...
이전까지는 WebSocket만을 사용해봤기 때문에 socket통신의 제약과 특징에 대해 이해할 수 있는 기회였습니다. 

그 중 특히 소켓 통신에서 수신과 송신이 각각의 독립적인 흐름으로 움직이고 있기 때문에 연결 관리와 더불어 공유 자원에 대한 접근제어의 중요성을 알 수 있었습니다.

협업 과정에서 얻은 점은 프로젝트 진행에 대한 협의와 의사소통이 얼마나 중요한가 였습니다. 
다양한 배경과 기술 스택을 가진 팀원들과 함께 프로젝트를 진행하기 위해서는 제가 알고있다고 넘기지 말고 세세히 공유하거나 확인해야 했습니다. 

팀원들 간의 의견이 공통된 상황에서 가장 완성도가 높은 결과물이 나올 수 있다는 점을 알 수 있는 기회였습니다.

마지막으로는 이번 프로젝트를 통해 기술적인 향상을 할 수 있었습니다.
소켓 통신에 대한 흥미와 이해도가 더욱 높일 수 있었으며
나아가서 시간 기반 버퍼링 방식을 도입해 네트워크 부담을 줄이거나, 유니캐스트 기능을 통해 귓속말 채팅을 처리하는 등의 다양한 리팩토링 아이디어를 얻는 기회였습니다.
